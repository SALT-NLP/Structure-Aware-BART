...
class BartEncoder(nn.Module):
    ...
    def forward(
        self, input_ids, attention_mask=None, output_attentions=False, output_hidden_states=False, return_dict=False, segmented_encoder = False,
    ):
        """
        Args:
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        static_kv: bool = self.encoder_decoder_attention or self.graph_decoder_attention or self.action_decoder_attention
...
class GAT(nn.Module):
    def __init__(self, config, nfeat, nhid, dropout = 0.2, alpha = 0.2, nheads = 2, non_relation = False):
        ...
        self.out_att = GraphAttentionLayer(nhid * nheads, nhid, dropout=dropout, alpha=alpha, concat=False, relation = self.relation)
...
class BartModel(PretrainedBartModel):
    def __init__(self, config: BartConfig):
        ...
        if config.action_graph:
            self.action_encoder = GAT(config, nfeat= config.d_model, nhid= config.d_model, nheads = config.action_encoder_attn_head, non_relation = True)
    ...
    def encode_action(self, actions, actions_mask, action_adj,  output_attentions=False,
        ...
        if not self.config.only_embedding:
            ...
            nodes = [torch.tensor(torch.zeros([action_adj.shape[1], node_representations.shape[-1]]), requires_grad=True).to(node_representations.device)]
    ...
    def encode_graph(self, utterance_representations, adj, input_ids, relation=False):
        ...
        sections = [torch.tensor(torch.zeros([adj.shape[1], utterance_representations.shape[-1]]), requires_grad=True).to(utterance_representations.device)]
        ...
        **kwargs,
        ...
        if self.config.action_graph:
            if action_output is None:
                action_output, action_padding_mask = self.encode_action(actions, actions_mask, action_adj, output_attentions=output_attentions,
                output_hidden_states=output_hidden_states,
                return_dict=return_dict,)
